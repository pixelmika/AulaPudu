...
class PresentationCreator {
  constructor(app) {
    ...
    // NEW: Propiedades para el resize
...
renderEditor() {
    ...
    activeSlide.content.forEach(element => {
      ...
      if (element.type === 'text') {
        ...
        textDiv.addEventListener('blur', (e) => {
          ...
          // Update element data in slides array
          const currentElement = activeSlide.content.find(item => item.id === element.id);
          if (currentElement) {
            currentElement.value = e.target.textContent;
          }
        });
  ...
  onResizeStart(e) {
    ...
    const element = handle.parentElement; // The .slide-element
    
    this.selectElement(element.dataset.elementId); // Select the element being resized

    this.isResizing = true;
    this.resizeHandle = handle;
    this.draggedElement = element; // Use draggedElement to refer to the element being manipulated
    ...
    e.preventDefault();
    e.stopPropagation(); // Prevent drag start from firing
  }

  onResize(e) {
    if (!this.isResizing || !this.draggedElement) return;

    const dx = e.clientX - this.initialMouseX;
    const dy = e.clientY - this.initialMouseY;
    const direction = this.resizeHandle.dataset.direction;
    ...
    if (direction.includes('e')) {
      newWidth = Math.max(20, this.initialWidth + dx);
    }
    if (direction.includes('s')) {
      newHeight = Math.max(20, this.initialHeight + dy);
    }
  ...
  onResizeEnd(e) {
    if (this.isResizing && this.draggedElement) {
      ...
      if (elementData) {
        elementData.x = parseInt(this.draggedElement.style.left, 10);
        elementData.y = parseInt(this.draggedElement.style.top, 10);
        elementData.width = parseInt(this.draggedElement.style.width, 10);
        elementData.height = parseInt(this.draggedElement.style.height, 10);
      }
    }
    this.isResizing = false;
    this.resizeHandle = null;
    this.draggedElement = null; // Clear draggedElement after resize
  }
