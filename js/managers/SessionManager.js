...
async startVideoPresentation(embedUrl) {
  try {
    ...
    // Configurar como presentación activa
    ...
    content.updateLiveUI();
    
    // Renderizar video en el presentador
    ...
    if (container) {
          ...
          ">
            <iframe 
              ...
              style="
                ...
                border: none;
              "
              onload="console.log('Iframe de video cargado correctamente')"
              onerror="console.error('Error cargando iframe de video')"
            ></iframe>
          </div>
      ...
      // Probar si el iframe se carga correctamente
      setTimeout(() => {
        const iframe = container.querySelector('iframe');
        if (iframe) {
          console.log('Iframe creado:', iframe.src);
        }
      }, 1000);
    }
...
handleVideoBroadcast(payload) {
  ...
  endPresentation() {
    ...
    document.getElementById('current-slide-content').innerHTML = '<h3>Diapositiva 1</h3><p>Carga una presentación.</p>';
