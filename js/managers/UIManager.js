...
initPresenterDashboard() {
  ...
  this.app.exams.loadExams(); // Ensure exams are loaded before updating stats
  this.updateOverviewStats(); // Call the new method here
...
initSidebarState() {
  ...
  async updateOverviewStats() {
    // Spectators
    const spectatorCount = Object.values(this.app.session.rt.liveSpectators).filter(s => s.type !== 'presenter').length;
    document.getElementById('overview-spectators-count').textContent = spectatorCount;

    // Presentations
    const presentationCount = await this.app.content.getPresentationCount();
    document.getElementById('overview-presentations-count').textContent = presentationCount;

    // Exams
    const examCount = await this.app.exams.getExamCount();
    document.getElementById('overview-exams-count').textContent = examCount;
  }
}
